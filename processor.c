...
void execute_one_inst(processor_t* p, int prompt, int print_regs)
{
  ...
  case 0x0: // opcode == 0x0 (SPECIAL)
    ...
    case 0x23: // funct == 0x21 (subu)
      p->R[inst.rtype.rd] = p->R[inst.rtype.rs] - p->R[inst.rtype.rt] ;
      p->pc += 4;
      break;

    case 0x24: // funct == 0x21 (and)
      p->R[inst.rtype.rd] = p->R[inst.rtype.rs] & p->R[inst.rtype.rt] ;
      p->pc += 4;
      break;
  ...
  case 0x4: // opcode == 0x4 (beq)
    if (p->R[inst.itype.rs] == p->R[inst.itype.rt])
      p->pc += 4 +  (int32_t)(int16_t)(inst.itype.imm)*4; 
    else
      p->pc += 4 ;
    break;
  ...
  case 0xa: // opcode == 0xa (slti)
    p->R[inst.itype.rt] = (signed)p->R[inst.itype.rs] < (int32_t)(int16_t)inst.itype.imm; 
    p->pc += 4;
    break;
  ...
  case 0x20: // opcode == 0x20 (lb)
    p->R[inst.itype.rt]=(int32_t)(int16_t)load_mem(p->R[inst.itype.rs]+(int32_t)(int16_t)inst.itype.imm,SIZE_BYTE);
    p->pc += 4;
    break;

  case 0x21: // opcode == 0x21 (lh)
    p->R[inst.itype.rt]=(int32_t)(int16_t)load_mem(p->R[inst.itype.rs]+(int32_t)(int16_t)inst.itype.imm,SIZE_HALF_WORD);
    p->pc += 4;
    break;
  ...
  case 0x25: // opcode == 0x25 (lhu)
    p->R[inst.itype.rt]=load_mem(p->R[inst.itype.rs]+(int32_t)(int16_t)inst.itype.imm,SIZE_HALF_WORD);
    p->pc += 4;
    break;
  ...
  case 0x29: // opcode == 0x29 (sh)
     store_mem(p->R[inst.itype.rs]+(int32_t)(int16_t)inst.itype.imm,SIZE_HALF_WORD,p->R[inst.itype.rt]);
    p->pc += 4;
    break;
